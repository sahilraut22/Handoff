 ...
 */
export function safePath(workingDir, userPath) {
    ...
    if (rel.startsWith('..') || rel.startsWith('/') || /^[A-Za-z]:/.test(rel)) {
        throw new SecurityError(ErrorCode.PATH_OUTSIDE_WORKSPACE, `Path "${userPath}" resolves outside working directory.`);
    }
    return resolvedPath;
}
/**
...
export function sanitizeAgentName(name) {
    if (!/^[a-zA-Z0-9_-]{1,64}$/.test(name)) {
        throw new SecurityError(ErrorCode.INVALID_AGENT_NAME, `Invalid agent name: "${name}".`);
    }
    return name;
}
 ...
 */
export function validateContentSize(content, maxBytes, label) {
    const bytes = Buffer.byteLength(content, 'utf-8');
    if (bytes > maxBytes) {
        throw new FileError(ErrorCode.FILE_TOO_LARGE, `${label} exceeds size limit: ${bytes.toLocaleString()} bytes (max ${maxBytes.toLocaleString()} bytes).`);
    }
}
 ...
 */
export function validateHandoffContent(content) {
