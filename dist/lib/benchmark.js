 ...
 */
export async function runBenchmark(name, fn, iterations = 50) {
    // Warmup
    for (let i = 0; i < 5; i++) {
        await fn();
    }
...
export function formatBenchmarkTable(results) {
    ...
    const colWidths = headers.map((_, ci) => Math.max(...allRows.map((row) => (row[ci] ?? '').length)));
    const separator = '+' + colWidths.map((w) => '-'.repeat(w + 2)).join('+') + '+';
    const formatRow = (row) => '| ' + row.map((cell, i) => cell.padEnd(colWidths[i] ?? 0)).join(' | ') + ' |';
    const lines = [
        separator,
        formatRow(headers),
        separator,
 ...
 */
export function formatBenchmarkJson(suite) {
    return JSON.stringify(suite, null, 2);
}
/**
 * Build a BenchmarkSuite from a list of results.
 */
export function buildBenchmarkSuite(results) {
    return {
        results,
        system: {
