...
export async function stopDaemon(workingDir, pidFile) {
...
export async function isDaemonRunning(workingDir, pidFile) {
    const pidPath = join(workingDir, pidFile ?? DEFAULT_PID_FILE);
    try {
        const content = await readFile(pidPath, 'utf-8');
...
export async function getDaemonStatus(workingDir) {
 ...
 */
export async function daemonEntryExists(entryPath) {
    try {
