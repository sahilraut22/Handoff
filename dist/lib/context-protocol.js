 ...
 */
export async function publishContext(ipcDir, handoffContent, updatedBy, sessionId = 'unknown') {
    ...
    for (const agent of activeAgents) {
        ...
        await sendMessage(ipcDir, msg).catch((err) => {
            logger.warn('Failed to notify agent of context update', { agent, error: err.message });
        });
    }
 ...
 */
export async function hasNewContext(ipcDir, agent, lastReadHash) {
    try {
 ...
 */
export async function readContext(ipcDir) {
    const ctxDir = contextDir(ipcDir);
    try {
 ...
 */
export async function acknowledgeContext(ipcDir, agent) {
    try {
        const metaContent = await readFile(join(contextDir(ipcDir), META_FILE), 'utf-8');
        const meta = JSON.parse(metaContent);
        const ackMsg = {
