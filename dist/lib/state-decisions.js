...
export async function loadTechState(workingDir) {
    try {
...
export async function saveTechState(workingDir, state) {
 ...
 */
function diffSnapshots(prev, curr, history) {
    const diffs = [];
 ...
 */
export async function runStateDetection(workingDir) {
    const current = await buildTechSnapshot(workingDir);
    // Nothing detected — no point comparing
    if (Object.keys(current.techs).length === 0)
        return [];
...
//# sourceMappingURL=state-decisions.js.map