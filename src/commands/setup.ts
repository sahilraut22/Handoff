...
import { installTmuxConfig, getConfigPath } from '../lib/tmux-config.js';
...
function checkCliInPath(cmd: string): boolean {
  try {
  ...
  program
    ...
    .action(async (options: {
      ...
      try {
        ...
        console.error(`\u2717 Failed to install config: ${(err as Error).message}`);
      }
