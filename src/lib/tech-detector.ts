...
interface TechCategory {
  label: string;
  tag: string;
  /** lowercase package/service names to match against */
  patterns: string[];
}

const CATEGORIES: Record<string, TechCategory> = {
  database: {
    label: 'database',
    tag: 'database',
    patterns: [
 ...
 */
async function detectFromPackageJson(workingDir: string): Promise<Record<string, string[]>> {
 ...
 */
async function detectFromPythonDeps(workingDir: string): Promise<Record<string, string[]>> {
  const found: Record<string, string[]> = {};
 ...
 */
async function detectFromDockerCompose(workingDir: string): Promise<Record<string, string[]>> {
  const found: Record<string, string[]> = {};
 ...
 */
async function detectFromEnvFile(workingDir: string): Promise<Record<string, string[]>> {
  const found: Record<string, string[]> = {};

  const URL_TECH_MAP: Record<string, string> = {
 ...
 */
async function detectFromImports(workingDir: string): Promise<Record<string, string[]>> {
  const found: Record<string, string[]> = {};
  ...
  for (const scanRoot of ['src', 'app', 'lib', 'server', '.']) {
    try {
      ...
      if (scanRoot !== '.') break; // found a src-like dir, stop
    } catch {
      // dir doesn't exist
    }
  }
 ...
 */
function mergeDetections(...sources: Array<Record<string, string[]>>): Record<string, string[]> {
 ...
 */
export async function buildTechSnapshot(workingDir: string): Promise<TechSnapshot> {
  ...
  for (const cat of Object.keys(techs)) {
    if (techs[cat]!.length === 0) delete techs[cat];
  }
