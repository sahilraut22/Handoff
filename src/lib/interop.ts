...
export async function detectMemoryFiles(
  dir: string
): Promise<Array<{ agent: string; file: string; path: string }>> {
  const found: Array<{ agent: string; file: string; path: string }> = [];
 ...
 */
export function generateClaudeMdSnippet(context: HandoffContext): string {
 ...
 */
export function generateAgentsMdSection(context: HandoffContext): string {
  const { session, changes, message } = context;
  const duration = formatDuration(session.created_at);
 ...
 */
export function generateJsonOutput(context: HandoffContext): string {
 ...
 */
export function generateInteropOutput(context: HandoffContext, format: InteropFormat): string {
  switch (format) {
    case 'json':
