...
function extractAlternatives(window: string[]): string[] {
  ...
  for (const m of listMatches.slice(0, 5)) {
    ...
    if (clean.length >= 5) alternatives.push(clean);
  }

  // "option A vs option B" patterns
  const vsMatch = joined.match(/(.{5,40})\s+(?:vs\.?|versus|or)\s+(.{5,40})/i);
  if (vsMatch && alternatives.length === 0) {
    if (vsMatch[1]) alternatives.push(vsMatch[1].trim());
    if (vsMatch[2]) alternatives.push(vsMatch[2].trim());
  }

  return alternatives.slice(0, 5);
}

function extractTitle(triggerLine: string, patternName: string): string {
...
function computeConfidence(
  ...
  if (triggerLine.trim().length < 30) confidence -= 0.1;

  // Penalize comment-only lines (aspirational, not actual decision)
  if (/^[+-]?\s*(?:\/\/|#|\/\*)/.test(triggerLine)) confidence -= 0.15;

  return Math.max(0, Math.min(1, confidence));
}

function deduplicateExtracted(decisions: ExtractedDecision[]): ExtractedDecision[] {
...
export function extractDecisions(
  ...
  if (config?.patterns) cfg.patterns = config.patterns;
  ...
  for (const pattern of cfg.patterns) {
    for (let i = 0; i < lines.length; i++) {
      const line = lines[i]!;
      if (!pattern.trigger.test(line)) continue;
      ...
      const tags: string[] = [];
      if (pattern.tag) tags.push(pattern.tag);
      ...
      for (let j = start; j <= end; j++) seenLineIndices.add(j);

      if (results.length >= cfg.max_decisions_per_scan) break;
    }
    if (results.length >= cfg.max_decisions_per_scan) break;
  }
...
): { new_decisions: ExtractedDecision[]; duplicates: number } {
  const normalizedExisting = existingTitles.map((t) =>
    new Set(extractQueryKeywords(t.toLowerCase()))
  );

  let duplicates = 0;
  const new_decisions: ExtractedDecision[] = [];
...
export function formatExtractedForReview(decisions: ExtractedDecision[]): string {
  if (decisions.length === 0) return 'No decisions extracted.';

  const lines: string[] = ['Extracted Decisions:', ''];
  for (const d of decisions) {
