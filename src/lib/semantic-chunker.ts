 ...
 */
export function chunkCode(content: string, language: string): SemanticChunk[] {
  ...
  for (const range of ranges) {
    ...
    let chunkType: SemanticChunk['type'] = 'other';
    if (entityType === 'function' || entityType === 'method') chunkType = 'function';
    else if (entityType === 'class') chunkType = 'class';
    else if (entityType === 'interface' || entityType === 'type') chunkType = 'interface';
 ...
 */
export function chunkDiff(diff: string, filePath: string): SemanticChunk[] {
  if (!diff.trim()) return [];
 ...
 */
export function scoreChunks(chunks: SemanticChunk[], query: string): SemanticChunk[] {
  if (!query || chunks.length === 0) return chunks;
 ...
 */
export function selectChunks(chunks: SemanticChunk[], tokenBudget: number): SemanticChunk[] {
  ...
  const selected: SemanticChunk[] = [...always];
 ...
 */
export function assembleChunks(chunks: SemanticChunk[], totalLines: number): string {
  if (chunks.length === 0) return '';
