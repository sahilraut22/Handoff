...
export async function walkFiles(dir: string, excludePatterns: string[]): Promise<string[]> {
  // ignore package exports differently in ESM vs CJS - handle both
  const ignoreFn = (ignoreModule as unknown as { default?: typeof ignoreModule }).default ?? ignoreModule;
...
export async function snapshotAllFiles(
  dir: string,
  files: string[],
  snapshotDir: string,
  concurrency = 50
  ...
  async function worker(): Promise<void> {
    while (queue.length > 0) {
      ...
      try {
        ...
        if (fileStat.size > MAX_SNAPSHOT_FILE_SIZE) continue;
...
function truncateDiff(diff: string, maxLines: number): { truncated: string; totalLines: number; wasTruncated: boolean } {
