...
// --- Entity ranges (for semantic chunking) ---
 ...
 */
export function extractEntityRanges(content: string, language: string): EntityRange[] {
  ...
  for (const entity of entities) {
    ...
    if (language === 'python') {
      ...
      for (let i = startLine + 1; i < lines.length; i++) {
        const line = lines[i] ?? '';
        if (line.trim() === '') continue; // blank lines allowed inside block
